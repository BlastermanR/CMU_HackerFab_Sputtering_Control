...
void handleSputterSpeed(pressure_measurement desired_pressure)
{
    ...
    reached_equilibrium(0.0, true,
                        desired_pressure.exp); // Reset equilibrium logic
    queryState = GAUGE;
    stage      = 2;
}
...
void updateFSM(pressure_measurement desired_pressure)
{
    ...
    if (stage == 1 && now - lastActionTime >= interval)
    {
        ...
        case EVAC:
            ...
            break;
        case SPUTTER_SPEED:
            handleSputterSpeed(desired_pressure);
            break;
        default:
            Serial.println("Unhandled state in Stage 1");
            break;
        }
        lastActionTime = now;
    }
    ...
    if (stage == 2 && now - lastActionTime >= sample_interval)
    {
        ...
        {
        case ALICAT:
            handleAlicat(desired_pressure);
            break;
        case GAUGE:
            handleGauge(desired_pressure);
            break;
        default:
            Serial.println("Unhandled state in Stage 2");
            break;
        }
        lastActionTime = now;
    }
