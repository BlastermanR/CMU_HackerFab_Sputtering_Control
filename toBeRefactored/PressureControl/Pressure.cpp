...
int count = 0
    ...
    // Waits for the user to input a desired pressure via serial in the format:
    // PSET,<frac>,<exp> Fills the desired_pressure struct with fractional,
    // exponent, and computed pressure
...
// Converts a 6-character string from the Pfeiffer gauge into a
// pressure_measurement struct Format expected: "XXXXYY" where XXXX is mantissa
// and YY is exponent offset from 20
pressure_measurement pressure_conversion(char *unconverted_pressure)
{
    if (strlen(unconverted_pressure) < 6)
        return {};
...
// Determines if pressure has reached equilibrium by evaluating standard
// deviation of recent readings If `clear` is true, the pressure window is reset
