/*
  ArduinoPfeiffer.cpp - Library for formatting Pfeiffer Vacuum pump commands.
...
ArduinoPfeiffer::ArduinoPfeiffer(ASCII_char address)
{
    _address = address;
...
ArduinoPfeiffer::~ArduinoPfeiffer()
{
    // No dynamic members in class itself; nothing to free
}

// Formats a complete Pfeiffer command including checksum and carriage return
ASCII_char ArduinoPfeiffer::pfeiffer_command_format(ASCII_char write, ASCII_char param_nums, ASCII_char data_len, ASCII_char data)
{
...
ASCII_char ArduinoPfeiffer::get_check_sum(char *without_checksum, uint8_t len)
{
    uint8_t check_sum = 0;
...
ASCII_char ArduinoPfeiffer::data_request(ASCII_char param_num)
{
    // "00" is the read mode for a data request, "02" is fixed length for "=?"
    return pfeiffer_command_format("00", param_num, "02", "=?");
}

// Creates a control command with given parameter number and data
ASCII_char ArduinoPfeiffer::control_request(ASCII_char param_num, ASCII_char data)
{
    ...
    return pfeiffer_command_format("10", param_num, data_len_char, data);
}

// Frees memory allocated for any ASCII_char message created by this class
void ArduinoPfeiffer::free_message(ASCII_char message)
{
    if (message) {
